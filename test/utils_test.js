...
describe("utils", () => {
  describe("getDefaultFormState()", () => {
    ...
    describe("nested default", () => {
      ...
      it("should use schema default for referenced definitions", () => {
        ...
        expect(
          getDefaultFormState(globalAjv, schema, undefined, schema.definitions)
        ).eql({
          foo: 42,
        });
      });
      ...
      describe("true condition", () => {
        describe("when required is not defined", () => {
          it("should define required properties", () => {
            ...
            const formData = { a: "1" };
            expect(
              retrieveSchema(globalAjv, schema, definitions, formData)
            ).eql({
              type: "object",
              properties: {
                a: { type: "string" },
  ...
  describe("toIdSchema", () => {
    ...
    it("should return an idSchema for property dependencies", () => {
      ...
      expect(
        toIdSchema(globalAjv, schema, undefined, schema.definitions, formData)
      ).eql({
        $id: "root",
        foo: { $id: "root_foo" },
        bar: { $id: "root_bar" },
    ...
    it("should handle idPrefix parameter", () => {
      ...
      expect(
        toIdSchema(globalAjv, schema, undefined, schema.definitions, {}, "rjsf")
      ).eql({
        $id: "rjsf",
        foo: { $id: "rjsf_foo" },
        bar: { $id: "rjsf_bar" },
  ...
  describe("isEqualExceptFunctions()", () => {
    ...
    it("should assume functions are always equivalent", () => {
      ...
      expect(
        isEqualExceptFunctions({ foo: { bar() {} } }, { foo: { bar() {} } })
      ).eql(true);
    });
  });
